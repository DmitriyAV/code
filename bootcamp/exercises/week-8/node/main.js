...
let biggestCountry = countries.reduce((previousValue, currentValue) => {
    return previousValue.population < currentValue.population ? currentValue : previousValue
})

console.log(biggestCountry.name)
...
for (let s of countries) {
    if (typeof (s.landmarks) !== "undefined")
    for (let l of s.landmarks) {
        if (l.tourismCount > 1000 || !"Unknown" || !typeof undefined) {
